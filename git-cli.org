...
*** show names of remotes
  git remote
*** show names and URLs of remotes
git remote -v
...
git pull -ff-only
*** fetch remote_branch from remote_name and rebase current branch onto it while favoring the content of the current local branch in any conflict
# yes, the ours vs theirs seems backwards
git pull --rebase -X theirs remote_name remote_branch
...
git branch -d -r <remote_name>/<remote_branch_name>
*** delete remote branch and its local remote-tracing branch
...
*** undo/remove/discard most recent one commit but leave their changes staged in the index and in the working tree
    git reset --soft HEAD^
    # This changes history. So if you already pushed that one commit to a shared repo, you'll have to force push this change.
*** undo/remove/discard most recent n commits and don't save their changes in the index or working tree
git rest --hard HEAD~n
# This changes history. So if you already pushed those n commits toa. shared repo, you'll have to force push this change.
...
git clean
*** discard(remove) untracked directories and files from working tree
git clean -d
...
have variables like
        remote=origin
        trunk=main
        feature_branch=mySpecialFeature
...
** UI like Clojure Quick Ref
find by box at top that filters based on typed text
outline / table of contents on left
clicking a task shows detailed description with steps to complete
variations show different ways to complete the same task
** modes
minimal mode
learning mode with extra explanations
